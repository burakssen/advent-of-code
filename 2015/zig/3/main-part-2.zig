...
pub fn main() !void {
    ...
    var santa: Position = Position{ .x = 0, .y = 0 };
    var robo_santa: Position = Position{ .x = 0, .y = 0 };
    var santa_turn = true;

    var visited = std.ArrayList(Position).init(std.heap.page_allocator);
    defer visited.deinit();

    try visited.append(santa);

    for (file_contents) |c| {
        if (santa_turn) {
            santa = santa.move(c);
            if (!hashPosition(visited.items, santa)) {
                try visited.append(santa);
            }
        } else {
            robo_santa = robo_santa.move(c);
            if (!hashPosition(visited.items, robo_santa)) {
                try visited.append(robo_santa);
            }
        }
