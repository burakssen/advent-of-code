        ...
        for i in action.start.0..=action.end.0 {
            for j in action.start.1..=action.end.1 {
                match action.act.as_str() {
                    "turn on" => self.grid[i as usize][j as usize] = true,
                    "turn off" => self.grid[i as usize][j as usize] = false,
                    "toggle" => {
...
fn main() {
    ...
    if args.len() != 2 {
        println!("Usage: cargo run --bin part1 <input.txt>");
        return;
    }
